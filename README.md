...
- **自动生成缩略图** - 同步时自动生成带水印 WebP 格式缩略图（800px），加速网页加载
- **自动生成预览图** - 同步时自动生成带水印预览图（1920px），保护原图版权
...
│   ├── mobile/             # 手机壁纸（手动管理）
│   └── avatar/             # 头像（手动管理）
├── thumbnail/              # 壁纸缩略图目录（WebP 格式，800px 宽，带水印）
│   ├── desktop/            # 电脑壁纸缩略图
│   ├── mobile/             # 手机壁纸缩略图
│   └── avatar/             # 头像缩略图
├── preview/                # 壁纸预览图目录（WebP 格式，1920px 宽，带水印）
│   └── desktop/            # 电脑壁纸预览图
├── blog/                   # 博客相关图片
├── docs/                   # 文档相关图片
├── projects/               # 项目相关图片
...
| **缩略图格式** | WebP，宽度 800px，质量 85%，带水印 |
| **预览图目录** | `preview/desktop/` |
| **预览图格式** | WebP，宽度 1920px，质量 90%，带水印 |
| **水印配置** | 文字"暖心"，右下角，-30° 倾斜，65% 透明度 |
| **缩略图水印** | 2% 字号，偏移 20x40 像素 |
| **预览图水印** | 3% 字号，偏移 40x80 像素 |
